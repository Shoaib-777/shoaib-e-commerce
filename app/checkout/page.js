...
const AddressCard = ({ savedAddresses, selectedAddress, setSelectedAddress, showNewAddress, setShowNewAddress, handleAddressSave, progressStage, setProgressStage }) => {
  return (
    <div className="bg-white rounded-xl shadow-lg p-6 mb-8">
      ...
      <div className="space-y-4">
        {savedAddresses.map((address) => (
          <div
            key={address.id}
            className={`border-2 rounded-lg p-4 cursor-pointer transition-all ${selectedAddress?.id === address.id
          ...
          >
            <div className="flex items-start justify-between">
              <div className="flex-1">
                ...
                <p className="text-gray-500 text-sm">{address.mobileNumber}</p>
              </div>
...
const OrderConfirmation = ({ onConfirm, orderTotal = 299.99, handleFinalOrder, progressStage, setProgressStage }) => {
  const [showAlert, setShowAlert] = useState(false);
  const orderItems = [
    { id: 1, title: "Blue T-Shirt Lorem ipsum dolor sit amet consectetur adipisicing elit. Hic, vero vitae soluta ullam veritatis nulla temporibus rerum aspernatur doloribus esse, expedita aliquam nihil est. In aut quisquam itaque assumenda ipsam?", quantity: 2, price: 29.99, image: "https://placehold.co/48x48?text=tshirt" },
    { id: 2, title: "Running Shoes", quantity: 1, price: 79.99, image: "https://placehold.co/48x48?text=shoes" },
  ]
          ...
          <div onClick={(e) => e.stopPropagation()} className="bg-white rounded-lg shadow-xl max-w-md w-full p-6">
            <div className="text-center mb-6">
              ...
              <p className="text-gray-600">
                Are you sure you want to place this order for ${orderTotal.toFixed(2)}?
              </p>
            </div>
...
const Checkout = () => {
  ...
  const [savedAddresses, setSavedAddresses] = useState([
    ...
    },
    {
      ...
      mobileNumber: '+91 9876543210',
      alternatePhone: '',
      addressLine1: '456 Business Park, Office 12',
      landmark: '',
      country: 'India',
      city: 'Mumbai',
      state: 'Maharashtra',
  ...
  const handleAddressSave = (newAddress) => {
    ...
    console.log([...savedAddresses, addressWithId])
  };
        ...
        <div className="bg-white rounded-lg shadow-xl border border-gray-300 p-8 text-center max-w-md w-full">
          ...
          <div className="bg-blue-50 border border-blue-200 rounded-lg p-3">
            <p className="text-sm text-blue-700">
              Order ID: <strong>DEMO-{newOrderId}</strong>
            </p>
          </div>
        </div>
      </div>
  ...
  return (
    <div className="max-w-4xl mx-auto p-2 sm:p-6 bg-gray-50 min-h-screen">
      ...
      {/* Address Selection */}
      {progressStage === 1 && (
        <AddressCard savedAddresses={savedAddresses} selectedAddress={selectedAddress} setSelectedAddress={setSelectedAddress} showNewAddress={showNewAddress} setShowNewAddress={setShowNewAddress} handleAddressSave={handleAddressSave} progressStage={progressStage} setProgressStage={setProgressStage} />
      )}
      ...
      {progressStage === 3 && (
        <OrderConfirmation onConfirm={handleOrderConfirm} handleFinalOrder={handleFinalOrder} progressStage={progressStage} setProgressStage={setProgressStage} />
      )}
    </div>
  );
};
